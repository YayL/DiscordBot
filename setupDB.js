...
pool.connect((e, client) => {
    ...
    client.query("CREATE TABLE users(key SERIAL PRIMARY KEY, id VARCHAR(22) NOT NULL , bank DECIMAL(30,0) DEFAULT 0, job VARCHAR(40) NOT NULL DEFAULT 'Unemployed', experience BIGINT NOT NULL DEFAULT 0, rebirths INT NOT NULL DEFAULT 0, inventory JSON, achivements JSON, gang VARCHAR(30))", (e) => { if (e) return console.error(e.stack) });
    ...
    client.query("CREATE TABLE gangs(name VARCHAR(30) NOT NULL PRIMARY KEY, owner VARCHAR(22) NOT NULL, members JSON, info JSON, vault JSON, bank DECIMAL(30,0) DEFAULT 0, experience BIGINT DEFAULT 0, tokens INT DEFAULT 0);", (e) => { if (e) return console.error(e.stack) });
    pool.disconnect();
});