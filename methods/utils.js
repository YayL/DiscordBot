...
module.exports = {
    ...
    timeFormat(totalSeconds) {
        ...
        str += `${(days > 0 || hours > 0 || min > 0 ) ? Math.ceil(seconds) : Math.ceil(seconds*10)/10}s`
    ...
    fixNumber(n = '0', is_money = false) {
        ...
        return (Number(n) < 0 ? '-' : '') + n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    },
        ...
        try {
            ...
            for (s of this.suffixList) {
                if (number.toLowerCase().endsWith(s.toLowerCase())) {
                    if (Number.isNaN(Number(number.slice(0, number.length - s.length))))
                        return false;
            ...
            if (Number.isNaN(Number(number)) || (!override && Number(number) < 1))
                return false;
