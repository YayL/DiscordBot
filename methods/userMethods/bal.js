async function setBalance(user_id, amount, client){
    const rows = client._user.get(client, user_id, '*')
    try{
        if(amount > client.s.MAX_MONEY) amount = client.s.MAX_MONEY
        if(amount < 0) amount=0

        client.con.query(`UPDATE user SET bal = ${amount} WHERE id = '${user_id}'`);
    }catch(e){
    ...
    addBalance(client, user_id, amount, set=false) {
        try{
            ...
            if(!set){
                ...
                .then(a => {
                    setBalance(user_id, a+amount, client);
                });
            }else{
