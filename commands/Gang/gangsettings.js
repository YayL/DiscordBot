module.exports = {
	...
	run: async function(msg, client, disc, args){
		try{
			...
			var info = JSON.parse(gang.info)

			if(args.length == 0) return sendSettings(client, msg, disc, gang, info.SETTINGS)

			if(Object.keys(info.SETTINGS).includes(args[0].toUpperCase())){
				var value = false
				if(args.length == 1) value = (info.SETTINGS[args[0].toUpperCase()] ? false : true)
				else value = (args[1] === 'true' ? true : false)
...
function sendSettings(client, msg, disc, gang, settings){
	...
	var settingKeys = Object.keys(settings), settingValues = Object.values(settings);

	for(var i = 0; i<settingKeys.length; i++){
		embed.addField(settingKeys[i], `**${settingValues[i]}**`, true)
	}
	
	embed.setTitle(`${JSON.parse(gang.info).NAME}'s Settings:`)
