module.exports = {
	...
	run: async function(msg, client, disc, args){
		try{
			// Above level 8
			if(await client._user.get(client, msg.author.id, 'job_xp') < client.data.jobs.totalLvlXp(8, true)) return client.eventEm.emit('TooLowLevel', msg, null, 8)
			// Name between 3 and 30 chars
			if(args.length == 0 || !( 3 <= args[0].length && args[0].length <= 30 ) ) return client.eventEm.emit('InvalidArgs', msg, this.use)
			...
			const name = args[0]
			// Guild name is not already in use
			if(!client.data.gang.isGang(client, name.toLowerCase())) return client.eventEm.emit('UnavailableName', msg, name)
