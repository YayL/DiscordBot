...
module.exports = {
	...
	run : async function(msg, client, disc, args){
		...
		try{
			...
			if(user.job_name == "Unemployed") return client.eventEm.emit('notEmployed', msg)

			if(client.data.cooldown.isOnCooldown(client, msg.author.id, 'work')) return client.eventEm.emit('WorkTimeout', msg, client.data.cooldown.getTimeLeft(client, msg.author.id, 'work'))

			var xp_grant = Math.floor(Math.random() * (xpTop - xpBottom + 1) + xpBottom)
			var base_pay = client.jobList.get(user.job_name).base_pay
			var money_grant = Math.floor(base_pay * ((0.6*Math.random())/3 + 1))*(user.rebirths+1)

			client.msg.reply(msg, "Great work!", `You earned **${xp_grant}xp** \n`
				+`and **$${client.utils.fixNumber(money_grant, true)}**`, disc)
